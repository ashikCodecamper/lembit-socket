...
io.on("connection", async function (socket) {
  ...
  if (isExists == 1) {
    try {
      ...
      io.broadcast('user_message',JSON.stringify({ type: "user_login", data: { user_id: userId } }));    } catch (err) {
      ...
      if (inactive.length) {
        inactive.map((userId) => {
          try {
            ...
            io.broadcast('user_message',JSON.stringify({ type: "user_logout", data: { user_id: userId } }));          } catch (err) {
